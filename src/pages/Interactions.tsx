...
import { Plus, Search, MessageSquare, Mail, Phone, Calendar, User, Trash2, Edit } from 'lucide-react';
...
const Interactions: React.FC = () => {
  ...
  const [selectedInteraction, setSelectedInteraction] = useState<Interaction | undefined>(undefined);
  ...
  const handleSubmitInteraction = async (interactionData: CreateInteractionRequest | UpdateInteractionRequest) => {
    try {
      setLoading(true);
      
      if ('id' in interactionData) {
        // It's an update
        await interactionsService.updateInteraction(interactionData.id, interactionData);
      } else {
        // It's a creation
        await interactionsService.createInteraction(interactionData.contact_id, {
  ...
  const filteredInteractions = interactions.filter(interaction => {
    ...
    return (
      ...
      (interaction.contact && interaction.contact.name.toLowerCase().includes(searchLower))
    );
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Registre e acompanhe todas as comunicações</p>
        </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    <div className={`p-2 rounded-md ${INTERACTION_COLORS[interaction.type]?.replace('text-', 'bg-').replace('-800', '-100')}`}>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${INTERACTION_COLORS[interaction.type]}`}>
